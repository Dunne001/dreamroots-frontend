...
import heroBg from '../assets/nairobi.jpg'
          ...
          <div style={{
            ...
            >
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
                  ...
                  <span className="label blue">Next Steps</span>
                </div>
                <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.6rem', fontWeight: 300, marginBottom: '1rem' }}>
                  What happens after you book?
                </h3>
                <ul style={{ display: 'flex', flexDirection: 'column', gap: '1rem', listStyle: 'none', color: 'var(--clr-text-muted)', fontSize: '0.9rem', lineHeight: 1.8 }}>
                  <li style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
                    <span style={{ color: 'var(--clr-blue)', fontWeight: 'bold' }}>1.</span>
                    <span><strong>Review:</strong> Our expert coordination team reviews your consultation request and preferred topics.</span>
                  </li>
                  <li style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
                    <span style={{ color: 'var(--clr-blue)', fontWeight: 'bold' }}>2.</span>
                    <span><strong>Matching:</strong> We pair you with the lead consultant specializing in your chosen service vertical.</span>
                  </li>
                  <li style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
                    <span style={{ color: 'var(--clr-blue)', fontWeight: 'bold' }}>3.</span>
                    <span><strong>Confirmation:</strong> Within 24 hours, you'll receive an email or call confirming the exact date, time, and session link.</span>
                  </li>
                </ul>
              </div>

              <div style={{ borderTop: '1px solid var(--clr-border)', paddingTop: '2rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
                  <span className="blue-line" />
                  <span className="label blue">FAQs</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                  {[
                    { q: 'Are consultation sessions free?', a: 'Our initial 30-minute discovery call is complimentary. Detailed strategic diagnostics are billed as per project engagement.' },
                    { q: 'Can we hold sessions virtually?', a: 'Yes. We host virtual consultation sessions via Microsoft Teams, Zoom, or Google Meet, or physical meetings at our Nairobi office.' },
                    { q: 'How should I prepare?', a: 'Prepare a brief overview of your organizational challenge, goals, and any relevant documents or data you want our experts to look at.' }
                  ].map((faq, idx) => (
                    <div key={idx}>
                      <h4 style={{ fontFamily: 'var(--font-sans)', fontSize: '0.88rem', fontWeight: 600, color: 'var(--clr-text)', marginBottom: '0.4rem' }}>{faq.q}</h4>
                      <p style={{ color: 'var(--clr-text-muted)', fontSize: '0.83rem', lineHeight: 1.6 }}>{faq.a}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div>
              {submitted ? (
                ...
                >
                  <div style={{ fontSize: '3rem', color: 'var(--clr-blue)', marginBottom: '1.5rem' }}>◆</div>
                  ...
                  <p style={{ color: 'var(--clr-text-faint)', fontSize: '0.8rem', marginBottom: '2rem' }}>
                    For urgent enquiries: <a href={`tel:${SITE.phone}`} style={{ color: 'var(--clr-blue)', textDecoration: 'none' }}>{SITE.phone}</a>
                  </p>
                  <button onClick={() => setSubmitted(false)} className="btn-blue"><span>Book Another</span></button>
                </motion.div>
              ) : (
                <motion.div
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '2.5rem' }}>
                    <span className="blue-line" />
                    <span className="label blue">Consultation Request</span>
                  </div>
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    ...
                    <button type="submit" className="btn-blue" disabled={isSubmitting} style={{ justifyContent: 'center', padding: '1rem 2rem', opacity: isSubmitting ? 0.7 : 1 }}>
