    ...
    <div style={{ paddingTop: '80px', minHeight: '100vh', backgroundColor: '#f8f9fa' }}>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '60px 20px' }}>
        {loading ? (
          ...
          <p style={{ textAlign: 'center', color: '#666' }}>No partners found.</p>
        ) : (
            ...
            {partners.map(function(partner) {
              return (
                <a key={partner.id} href={partner.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                  <div
                    style={{ backgroundColor: '#fff', borderRadius: '12px', padding: '32px 24px', boxShadow: '0 2px 12px rgba(0,0,0,0.08)', textAlign: 'center', transition: 'transform 0.2s', cursor: 'pointer' }}
                    onMouseEnter={function(e) { e.currentTarget.style.transform = 'translateY(-4px)'; }}
                    onMouseLeave={function(e) { e.currentTarget.style.transform = 'translateY(0)'; }}
                  >
                    <div style={{ fontSize: '2.5rem', marginBottom: '16px' }}>🤝</div>
                    <h3 style={{ color: '#1B4F72', fontSize: '1rem', lineHeight: '1.5' }}>{partner.name}</h3>
                    <p style={{ color: '#2E86C1', fontSize: '0.85rem', marginTop: '8px' }}>Visit Website</p>
                  </div>
                </a>
              );
            })}
